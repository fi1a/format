...
## Всегда используемые функции спецификаторы при форматировании

Функция спецификатор `escape` всегда используется при форматировании и экранирует выводимые значения.
Вы можете добавить свои функции спецификаторы, которые будут всегда использоваться при форматировании.

Метод `useAlways` первым аргументом принимат функцию спецификатор или сокращение, вторым - значение сортировки, определяющее
в какой последовательности будут расположены функции спецификаторы или сокращении в цепочке.

```php
use Fi1a\Format\Formatter;

Formatter::useAlways('sprintf("\'.9d")', 600);
Formatter::useAlways('~spf');
```

Доступны следующие методы по управлению всегда используемыми функциями сперцификаторами:

| Метод                                               | Описание                                                                                |
|-----------------------------------------------------|-----------------------------------------------------------------------------------------|
| useAlways(string $specifier, int $sort = 500): bool | Всегда использовать спецификатор при форматировании                                     |
| hasAlways(string $specifier): bool                  | Проверяет наличие спецификатора, который будет всегда использоватьмя при форматировании |
| unuseAlways(string $specifier): bool                | Удаляет спецификатор, который всегда используется при форматировании                    |
| getAlways(): array                                  | Возвращает все спецификаторы,  которые всегда используются при форматировании           |
